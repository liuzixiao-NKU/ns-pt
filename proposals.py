...
def _update_kwargs(**kwargs):
    ...
    mvn_object = multivariate_normal(np.zeros(self.dimension),self.covariance.T)
...
def _EnsembleStretch(inParam,**kwargs):
    ...
    if (Z<1.0/self.scale)or(Z>self.scale): log_acceptance_probability = -np.inf
    else: log_acceptance_probability = np.log(np.random.uniform(0.,1.))-(self.dimension)*np.log(Z)
    return inParam,log_acceptance_probability
