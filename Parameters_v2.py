...
def computeLogLinj(pulsars):
  ...
  for n in pulsars[0].pars:
    if pulsars[0].prefit[n].val == pulsars[0].prefit[n].val:
      pulsars[0][n].val = np.copy(pulsars[0].prefit[n].val)
      pulsars[1][n].val = np.copy(pulsars[1].prefit[n].val)
    else:
      for p in pulsars:
...
class Parameter(object):
  ...
  """
  def __init__(self,pulsars,model='Free'):
    self.model = model
...
#      gp = george.GP(sigma * kernels.ExpSquaredKernel(tau))
#gp.compute(t, data.flux_error)#np.sqrt(data.flux_error**2+0.0001))
#  self.logL = gp.lnlikelihood(data.flux-flux_model)-100000.0
